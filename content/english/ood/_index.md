...
>__3.__ Use your Alliance (Compute Canada) username and password to log in to Grex OOD.
...
There are different option for MFA, like the 6 digits passcode generated by DUO mobile application:
...
A lisf of ineractive applications is accessivle from the top menu __Interactive Apps__ as shown in the following screenshot:

{{< collapsible title="File view on OpenOndemand web portal on Grex" >}}
![OpenOnDemand Interactive Apps](/ood/interactive-apps0.png)
{{< /collapsible >}}

If you scroll down from the front page, some icons with links to different applications are shown:
...
{{< alert type="warning" >}}
Please note that the list of applications may change over time. The snapshots shown on this page are taken from the current liste of the available applications on the OpenOnDemand portal.
{{< /alert >}}
